-- Application state store tables (api_keys, users, logs, traces, query_cache,
-- audit_log), applied here -- alongside 0001's auth coordination
-- tables -- when the application state store shares this same Postgres
-- database instead of living in a separate SQLite file (see
-- core.storage.application_state_store's module docstring). Same schema as
-- the legacy SQLite-era initial schema migration, translated to
-- Postgres's dialect:
...
-- unchanged, so application code (core/storage/application_state_store.py and the
